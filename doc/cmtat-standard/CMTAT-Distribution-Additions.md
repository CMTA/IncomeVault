...
These three are not defects in an implementation — they are questions the specification leaves unanswered, and an issuer has to answer all three to discharge a real obligation. A-1 depends on the claim period actually closing, which is C-2 in the amendments; A-3 follows from A-2, since a push payout and a delegated claim answer the same problem for the same holders.
