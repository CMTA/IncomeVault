...
| C-9 | **27, 29** | **Say whether the deposit must be held as the settlement token**, and what `amount` means when that token is itself a vault share | Nothing constrains an implementation to hold the deposit idle, or the settlement token to be a plain ERC-20. Holding the float as ERC-4626 shares turns a fixed nominal obligation into one the contract may be unable to pay in full; distributing a vault share leaves "amount" ambiguous between shares and assets, with opposite rounding conventions on the two sides. See [*Holding a distribution deposit in an ERC-4626 vault*](./CMTAT-Distribution-ERC4626.md). |
