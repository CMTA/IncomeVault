...
- `doc/coverage/` is generated and **git-ignored** — regenerate with `make coverage-report`. It is not committed because a stale report is worse than none: this repo carried the *RuleEngine* project's coverage output for months. `forge coverage` **does** work — `make coverage` for a summary, `make coverage-report` for HTML in `doc/coverage`. An earlier note here claimed it did not, because the tests deploy through a proxy; that was checked and is wrong. Two files report 0% because they declare hooks with no bodies (`IncomeVaultSnapshotCore`, `IncomeVaultValidationCore`), which is expected rather than a gap.
