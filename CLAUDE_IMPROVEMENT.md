...
Generated by `solidity-docgen` before the CMTAT v3 migration; it predated the base/variant split, the
ERC-7201 storage, the authorization hooks and `ISnapshotSource`.

Regenerated: 8,737 -> 76,812 bytes, zero references to `ICMTATSnapshot`, `IAuthorizationEngine`,
`CMTAT_TOKEN` or `__gap`, and it now covers `IncomeVaultBaseERC2771`, `IncomeVaultOperatorModule`,
`IncomeVaultSnapshotCore`, `IIncomeVault` and `dividendSnapshotSource`.

Regenerating first required two fixes, both of which are why it went stale and stayed stale:

- **`npx hardhat docgen` did not run at all.** It aborted with
  `ItemError: Expected @return tag to start with name '$'` on the four ERC-7201 storage accessors.
  `solidity-docgen` will not accept a `@return` naming the `$` return variable — and neither does the
  project's own style checker, which reports the same four as `missing-return`. OpenZeppelin leaves
  these accessors undocumented entirely; the `@return $ the storage struct` lines are removed and the
  `@dev` kept.
- **The output landed in `docs/`, not `doc/solidityAPI/`.** `hardhat.config.js` had no `docgen`
  section, so regenerating meant generating into the wrong directory and moving the file by hand — a
  step easy to skip, and skipping it is what leaves the committed copy stale. `outputDir` and
  `pages: 'single'` are now configured, so the command writes straight to its committed location.

While there: `hardhat.config.js` declared `settings` at the **top level**, where Hardhat ignores it, so
